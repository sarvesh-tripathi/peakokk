...
public class AuthenticationProcessingFilter extends UsernamePasswordAuthenticationFilter
{
            ...
            Authentication authResult) throws ServletException, IOException
    {
        ...
        Cookie dbSessionCookie = new Cookie("DBSESSION", userDetailsService.getDbSessionId());
        dbSessionCookie.setPath("/peakokk/app");
        dbSessionCookie.setMaxAge(14*24*60*60);
        response.addCookie(dbSessionCookie);
        request.getSession().setAttribute("user", user);
        request.getSession().setAttribute(cookieName, currentCookie);
        request.getSession().setAttribute("sessionExist", userDetailsService.isSessionExist());
        request.getSession().setAttribute("USERID", userDetailsService.getUserId());
        request.getSession().setAttribute("USERNAME", userDetailsService.getUserName());
        request.getSession().setAttribute("USEREMAIL", userDetailsService.getUserEmail());
        request.getSession().setAttribute("USER_TYPE_ID", userDetailsService.getUserTypeId());
    ...
    public AuthenticationService getUserDetailsService()
    {
        return userDetailsService;
    }

    public void setUserDetailsService(AuthenticationService userDetailsService)
    {
